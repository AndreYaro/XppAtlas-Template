...
| `reference_paths` | Logical MCP source references for loading model code with `resolve_object`, `get_chunk`, and `get_file` |
...
| **Source control** | Never modify or rely on local `Source/` folders; load source through MCP and copy only the needed artifact into the task `code/` folder |
...
3. If relevant, load the source artifact through D365 MCP, then copy the needed XML into `Tasks/{TaskName}/code/Ax*/` before editing.
